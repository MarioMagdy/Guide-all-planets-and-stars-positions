...
observer_time = datetime.datetime(2023, 4, 27, 0, 0, 0)  # Cairo time
...
for planet in planets:
    ...
    x_ecl = x_orb * (math.cos(N) * math.cos(v + p) - math.sin(N) * math.sin(v + p) * math.cos(i))
    y_ecl = x_orb * (math.sin(N) * math.cos(v + p) + math.cos(N) * math.sin(v + p) * math.cos(i))
    z_ecl = x_orb * (math.sin(v + p) * math.sin(i))
...
for planet in planets:
    ...
    print(f"Altitude: {altaz_positions[planet]['alt'] * RAD_TO_DEG:.5f} deg")
    print(f"Azimuth: {altaz_positions[planet]['az'] * RAD_TO_DEG:.5f} deg")
    print(f"Right Ascension: {positions[planet]['ra'] * RAD_TO_DEG:.5f} deg")
    print(f"Declination: {positions[planet]['dec'] * RAD_TO_DEG:.5f} deg")
    print()
